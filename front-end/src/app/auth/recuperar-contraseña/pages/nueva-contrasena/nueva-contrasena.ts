...
import {
  SecureAuthService,
  CambiarPasswordRecuperacionRequest,
} from '../../../../services/secure-auth.service';
...
export class PageNuevaContrasena implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    if (this.token && this.token.length !== 6) {
      console.error(
        `❌ Token inválido - debe ser de 6 dígitos, tiene: ${this.token.length} dígitos`,
      );
      this.showTooltipMessage(
        `Código inválido (${this.token.length} dígitos). Debe ser de 6 dígitos`,
        'error',
      );
      setTimeout(() => {
